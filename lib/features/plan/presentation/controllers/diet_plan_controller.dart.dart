...
class DietPlanController extends BasePlanController {
  ...
  void initializeForm() async {
    ...
    if (userId.value.isNotEmpty) {
      ...
      if (client != null) {
        ...
        mealCalorieControllers['Breakfast']!.text =
            suggestedCalories['Breakfast'].toString();
        mealCalorieControllers['Morning Snack']!.text =
            suggestedCalories['Morning Snack'].toString();
        mealCalorieControllers['Lunch']!.text =
            suggestedCalories['Lunch'].toString();
        mealCalorieControllers['Evening Snack']!.text =
            suggestedCalories['Evening Snack'].toString();
        mealCalorieControllers['Dinner']!.text =
            suggestedCalories['Dinner'].toString();
      }
    }
    ...
    if (isEditMode.value && args?['plan'] != null) {
      ...
      planMeals.forEach((mealName, mealData) {
        meals[mealName] = {
          ...
          'fats': mealData['fats']?.toDouble() ?? 0.0,
          'foods': (mealData['foods'] as List? ?? []).map((food) {
            return {
              ...
              'protein': food['protein']?.toString() ?? '0.0',
              'carbs': food['carbs']?.toString() ?? '0.0',
              'fats': food['fats']?.toString() ?? '0.0',
              'controllers': {
                ...
                'protein': TextEditingController(text: food['protein']?.toString() ?? '0.0'),
                'carbs': TextEditingController(text: food['carbs']?.toString() ?? '0.0'),
                'fats': TextEditingController(text: food['fats']?.toString() ?? '0.0'),
              },
            };
          }).toList(),
          'controllers': {
            'calories': TextEditingController(text: mealData['calories']?.toString() ?? '0'),
            'protein': TextEditingController(text: mealData['protein']?.toString() ?? '0.0'),
            'carbs': TextEditingController(text: mealData['carbs']?.toString() ?? '0.0'),
            'fats': TextEditingController(text: mealData['fats']?.toString() ?? '0.0'),
          },
        };
  ...
  void addMeal() {
    ...
    if (userData.value != null) {
      ...
      if (customCount > 0 && total > 0) {
        mealCalorieControllers[mealName]!.text = (total * 0.125 / customCount)
            .round()
            .toString();
      }
    }
  ...
  Future<bool> removeMeal(String mealName) async {
    ...
    final controllers =
        meals[mealName]!['controllers'] as Map<String, TextEditingController>;
  ...
  Future<bool> removeFood(String mealName, int foodIndex) async {
    ...
    if (!confirmed) return false;
    final fc =
        meals[mealName]!['foods'][foodIndex]['controllers']
  ...
  Future<bool> savePlan() async {
    ...
    final totalCalories =
        int.tryParse(totalCaloriesController.text.trim()) ?? 0;
    final totalMealCalories = meals.entries.fold<int>(
      0,
      (sum, entry) =>
          sum +
          (int.tryParse(mealCalorieControllers[entry.key]?.text ?? '0') ?? 0),
    );
    if (totalMealCalories > totalCalories) {
      error.value =
          'Total meal calories ($totalMealCalories) exceed plan’s total ($totalCalories)';
      Get.snackbar(
        'Error',
        error.value,
        backgroundColor: AdminTheme.colors['error'],
    ...
    final plan = PlanModel(
      ...
      details: {
        ...
        'meals': meals.map(
          (mealName, meal) => MapEntry(mealName, {
            'calories':
                int.tryParse(mealCalorieControllers[mealName]?.text ?? '0') ??
                0,
            'protein':
                double.tryParse(meal['controllers']['protein'].text.trim()) ??
                0.0,
            'carbs':
                double.tryParse(meal['controllers']['carbs'].text.trim()) ??
                0.0,
            'fats':
                double.tryParse(meal['controllers']['fats'].text.trim()) ?? 0.0,
            'foods': (meal['foods'] as List).map((food) {
              return {
                ...
                'unit': food['unit'] ?? 'g',
                'calories':
                    int.tryParse(food['controllers']['calories'].text.trim()) ??
                    0,
                'description': food['controllers']['description'].text.trim(),
                'protein':
                    double.tryParse(
                      food['controllers']['protein'].text.trim(),
                    ) ??
                    0.0,
                'carbs':
                    double.tryParse(food['controllers']['carbs'].text.trim()) ??
                    0.0,
                'fats':
                    double.tryParse(food['controllers']['fats'].text.trim()) ??
                    0.0,
              };
            }).toList(),
          }),
    ...
    try {
      ...
      if (success) {
        ...
        Get.snackbar(
          'Success',
          '$planType plan ${isEditMode.value ? 'updated' : 'assigned'} successfully',
